...
\begin{example}[Signals as vectors]
Fix sample points $t_1,\ldots,t_N$. A signal sampled at those points is a list
\[
  (f(t_1),\ldots,f(t_N))\in\R^N.
\]
Adding signals pointwise and multiplying them by scalars gives another signal.
This is the finite-dimensional shadow of a function space.
\end{example}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{book/assets/figures/core/fig02_subspace.png}
  \caption{A subspace must pass through the origin and be closed under vector
  addition and scaling. An affine shift may look linear, but it is not a
  subspace.}
\end{figure}
...
If $U,W\subseteq V$ are subspaces, their sum is
\[
...
\begin{definition}
The sum $U+W$ is a \emph{direct sum}, written $U\oplus W$, if every vector in
$U+W$ can be written in exactly one way as $u+w$ with $u\in U$ and $w\in W$.
\end{definition}

For two subspaces, the uniqueness condition is equivalent to
\[
  U\cap W=\{0\}.
\]
Direct sums are the formal version of decomposing a vector into independent
pieces. Later, orthogonal decompositions will let us split a vector into a
part we can explain and a residual part we cannot.
...
\input{book/exercises/ch02-exercises}
